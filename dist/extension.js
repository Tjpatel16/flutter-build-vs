...
var require_filesystem = __commonJS({
  "node_modules/detect-libc/lib/filesystem.js"(exports2, module2) {
    ...
    var readFileSync3 = (path4) => fs4.readFileSync(path4, "utf-8");
    var readFile = (path4) => new Promise((resolve, reject) => {
      fs4.readFile(path4, "utf-8", (err, data) => {
        if (err) {
          reject(err);
        } else {
          resolve(data);
...
var require_output = __commonJS({
  "node_modules/sharp/lib/output.js"(exports2, module2) {
    ...
    function toFile(fileOut, callback) {
      ...
      } else if (is.string(this.options.input.file) && path4.resolve(this.options.input.file) === path4.resolve(fileOut)) {
...
var FLUTTER_PROJECTS_KEY = "flutterProjects";
var globalState;
function initializeFileUtils(context) {
...
async function addFlutterProject() {
...
function getCurrentAppDetails(projectPath) {
  try {
    let appName = "Unknown";
    ...
    if (appName === "Unknown") {
      ...
      if (fs.existsSync(pubspecPath)) {
        ...
        appName = pubspecContent.match(/name:\s*(.*)/)?.[1]?.trim() || "Unknown";
      }
    }
    let packageName = "";
    ...
    if (!packageName) {
      packageName = `com.example.${appName.toLowerCase().replace(/[^a-z0-9]/g, "")}`;
    }
    return {
      appName,
      packageName,
...
async function removeFlutterProject(projectPath) {
...
// src/providers/TabManager.ts
var vscode4 = __toESM(require("vscode"));

// src/providers/AppIconPanel.ts
var AppIconPanel = class {
  static getTabContent(projectPath) {
    return `
            <!DOCTYPE html>
            <html lang="en">
            <head>
...
function renameFlutterProject(projectPath, newAppName, newPackageName, platforms) {
  try {
    if (platforms.android) {
      const gradlePath = path2.join(projectPath, "android", "app");
      const buildGradlePath = path2.join(gradlePath, "build.gradle");
      const buildGradleKtsPath = path2.join(gradlePath, "build.gradle.kts");
      if (fs2.existsSync(buildGradleKtsPath)) {
        let gradleContent = fs2.readFileSync(buildGradleKtsPath, "utf8");
        gradleContent = gradleContent.replace(
          /applicationId\s*=\s*"[^"]+"/g,
          `applicationId = "${newPackageName}"`
        );
        fs2.writeFileSync(buildGradleKtsPath, gradleContent, "utf8");
      } else if (fs2.existsSync(buildGradlePath)) {
        let gradleContent = fs2.readFileSync(buildGradlePath, "utf8");
        gradleContent = gradleContent.replace(
          /applicationId\s*=\s*["'][^"']+["']/g,
          `applicationId = "${newPackageName}"`
        );
        fs2.writeFileSync(buildGradlePath, gradleContent, "utf8");
      }
      ...
      if (fs2.existsSync(manifestPath)) {
        let manifestContent = fs2.readFileSync(manifestPath, "utf8");
        manifestContent = manifestContent.replace(
          /android:label=["'][^"']*["']/g,
          `android:label="${newAppName}"`
        );
        fs2.writeFileSync(manifestPath, manifestContent, "utf8");
      }
    }
    if (platforms.ios) {
      const iosPlistPath = path2.join(
        projectPath,
        "ios",
        "Runner",
        "Info.plist"
      );
      if (fs2.existsSync(iosPlistPath)) {
        let plistContent = fs2.readFileSync(iosPlistPath, "utf8");
        plistContent = plistContent.replace(
          /(<key>CFBundleDisplayName<\/key>\s*<string>)[^<]+(<\/string>)/,
          `$1${newAppName}$2`
        );
        fs2.writeFileSync(iosPlistPath, plistContent, "utf8");
        const pbxprojPath = path2.join(
          projectPath,
          "ios",
          "Runner.xcodeproj",
          "project.pbxproj"
        ...
        if (fs2.existsSync(pbxprojPath)) {
          let pbxprojContent = fs2.readFileSync(pbxprojPath, "utf8");
          pbxprojContent = pbxprojContent.replace(
            /PRODUCT_BUNDLE_IDENTIFIER = [^;]+;/g,
            `PRODUCT_BUNDLE_IDENTIFIER = ${newPackageName};`
          );
          fs2.writeFileSync(pbxprojPath, pbxprojContent, "utf8");
        }
      }
    }
    ...
    vscode2.window.showInformationMessage(`Project updated successfully!
              App Name: ${newAppName}
              Package Name: ${newPackageName}
              Platforms: ${platformsUpdated.join(", ")}`);
  } catch (error) {
    vscode2.window.showErrorMessage(`Error updating project: ${error}`);
    throw error;
  }
}

// src/providers/RenamePanel.ts
var RenamePanel = class {
  static getTabContent(projectPath) {
  ...
  static handleRenameMessage(projectPath, message) {
...
// src/utils/AppIconUtils.ts
var fs3 = __toESM(require("fs"));
var path3 = __toESM(require("path"));
var vscode3 = __toESM(require("vscode"));
async function updateAppIcon(projectPath, logoData, platforms) {
  try {
    const tempDir = path3.join(projectPath, ".flutter-build-temp");
    if (!fs3.existsSync(tempDir)) {
      fs3.mkdirSync(tempDir);
    }
    const originalImagePath = path3.join(tempDir, "original_icon.png");
    fs3.writeFileSync(originalImagePath, Buffer.from(logoData, "base64"));
    if (platforms.android) {
    ...
    fs3.unlinkSync(originalImagePath);
    ...
    vscode3.window.showErrorMessage(`Error updating app icon: ${error}`);
    throw error;
  }
}
async function updateAndroidIcons(projectPath, originalImagePath) {
  const mipmapDirs = [
  ...
  const sharp = await Promise.resolve().then(() => __toESM(require_lib()));
  for (const { dir, size } of mipmapDirs) {
    const mipmapPath = path3.join(
      projectPath,
      "android",
      "app",
      "src",
    ...
    if (!fs3.existsSync(mipmapPath)) {
      fs3.mkdirSync(mipmapPath, { recursive: true });
    }
    await sharp.default(originalImagePath).resize(size, size).toFile(path3.join(mipmapPath, "ic_launcher.png"));
    await sharp.default(originalImagePath).resize(size, size).toFile(path3.join(mipmapPath, "ic_launcher_round.png"));
  }
  const adaptiveIconPath = path3.join(
    projectPath,
    "android",
    "app",
    "src",
  ...
  if (!fs3.existsSync(adaptiveIconPath)) {
    fs3.mkdirSync(adaptiveIconPath, { recursive: true });
  }
  ...
  fs3.writeFileSync(
    path3.join(adaptiveIconPath, "ic_launcher.xml"),
    adaptiveIconXml
  );
  fs3.writeFileSync(
    path3.join(adaptiveIconPath, "ic_launcher_round.xml"),
    adaptiveIconXml
  );
  const foregroundPath = path3.join(
    projectPath,
    "android",
    "app",
    "src",
  ...
  await sharp.default(originalImagePath).resize(192, 192).extend({
    top: 48,
    bottom: 48,
    left: 48,
    right: 48,
    background: { r: 0, g: 0, b: 0, alpha: 0 }
  }).toFile(path3.join(foregroundPath, "ic_launcher_foreground.png"));
  const colorPath = path3.join(
    projectPath,
    "android",
    "app",
    "src",
  ...
  if (!fs3.existsSync(colorPath)) {
    fs3.mkdirSync(colorPath, { recursive: true });
  }
  ...
  fs3.writeFileSync(
    path3.join(colorPath, "ic_launcher_background.xml"),
    colorXml
  );
}
async function updateIOSIcons(projectPath, originalImagePath) {
  const iosIconSizes = [
  ...
  if (!fs3.existsSync(appIconPath)) {
    fs3.mkdirSync(appIconPath, { recursive: true });
  }
  const contentsJson = {
    images: [],
    info: {
  ...
  for (const { size, scales } of iosIconSizes) {
    for (const scale of scales) {
      ...
      await sharp.default(originalImagePath).resize(pixelSize, pixelSize).toFile(path3.join(appIconPath, filename));
    }
  }
  fs3.writeFileSync(
    path3.join(appIconPath, "Contents.json"),
...
var TabManager = class {
  static currentTabs = /* @__PURE__ */ new Map();
  static show(projectPath, action) {
    ...
    const panel = vscode4.window.createWebviewPanel(
      "flutterBuildTab",
      `${projectName} - ${action === "rename" ? "Rename App" : "Change App Icon"}`,
  ...
  static getTabHtml(projectPath, action) {
    const content = action === "rename" ? RenamePanel.getTabContent(projectPath) : AppIconPanel.getTabContent(projectPath);
    return content;
  }
};
...
var SidebarProvider = class {
  ...
  static viewType = "flutter-build-view";
  _view;
                ...
                <div class="project-item" onclick="showProjectMenu('${p}')">
                    <div class="project-info">
                        ...
                        <div class="project-details">
                            <span class="project-name">${p.split("/").pop()}</span>
    ...
    webviewView.webview.onDidReceiveMessage(async (message) => {
      switch (message.command) {
        ...
        case "createNewProject":
          await vscode5.commands.executeCommand("flutter.createProject");
          break;
        case "showProjectMenu":
          const items = [
            {
              label: "Rename App Name & Package",
              description: "Rename the app name and package name of the project",
              type: "rename"
            },
            {
  ...
  _getWebviewContent(projectList, projects) {
...
function activate(context) {
  ...
  context.subscriptions.push(
    vscode6.commands.registerCommand("flutter-build.start", (projectPath) => {
